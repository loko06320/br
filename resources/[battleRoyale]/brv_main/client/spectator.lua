    ...
    if getIsGameStarted() and isPlayerInLobby() then
      ...
      if isPlayerNearCoords(tvCoords, 100.0) then
        ...
        if isPlayerNearCoords(tvCoords, 2.5) then
          inTVZone = true
          showHelp('Appuie ~INPUT_ENTER~ pour entrer en mode Spectateur')
        else
          inTVZone = false
        end
      else
        ...
        if alivePlayers[index] ~= nil then
          ...
          drawInstructionalButtons({
            ...
            },
            {
              ...
              label = 'Precedent joueur'
            },
          })
          showText('Spectateur ' .. alivePlayers[index].name, 0.45, 0.05, conf.color.grey)
          showText('Santé : ' .. tostring(GetEntityHealth(GetPlayerPed(playerToSpec))), 0.45, 0.08, conf.color.green)
        else
